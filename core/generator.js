...
class CodeGenerator {
    ...
    generateFiles(filePath) {
        ...
        for (let i = 0; i < this.entities.length; i++) {
            ...
            const entityRelationship = this.relationships
                .filter(x => x.end1.table === entity.name || x.end2.table === entity.name);

            let entityWriter = new writer.EntityWriter(entity, entityRelationship)
            fs.writeFileSync(filePath + `\\${entity.name}Entity.cs`, entityWriter.getData(this.relationships));
        }
    }
}

function generateCode(diagram, filePath) {
    console.log(diagram);
    const generator = new CodeGenerator();
    generator.generate(diagram, filePath);
}
